...
def p_Template_error(p):
    "Template : error"
    print(f"Syntax error in template. Bad everything, line {p.lineno(3)}")
    p.parser.success = False


def p_Expression_error(p):
    "Expression : OE error CE"
    print(f"Syntax error in expression block. Bad expression, line {p.lineno(2)}")
    p.parser.success = False


def p_If_error(p):
    "If : OS IF error CS Elems OS ENDIF CS"
    print(f"Syntax error in if block. Bad expression, line {p.lineno(3)}")
    p.parser.success = False
...
def p_Elif_error(p):
    ...
    print(f"Syntax error in elif block. Bad expression, line {p.lineno(3)}")
    p.parser.success = False


def p_For_error(p):
    "For : OS FOR error IN id CS Elems OS ENDFOR CS"
    print(f"Syntax error in for block. Bad id, line {p.lineno(3)}")
    p.parser.success = False
